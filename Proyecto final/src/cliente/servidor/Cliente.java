...
public class Cliente extends JFrame implements ActionListener {
	...
	protected Datos obtenerDatos() throws SigarException, UnknownHostException {
		...
		datos = new Datos(cliente, modeloProcesador, velocidadProcesador, so, ram, discoTotal, cpuLibre, ramLibre,
				discoLibre);
						...
						public void run() {
							while (!parar) {
								try {
									obtenerDatos();
									Thread.sleep(4000);
									enviarDatos(txtIPdestino.getText().toString(),
											Integer.parseInt(txtPuerto.getText()));

								} catch (IOException | NumberFormatException | SigarException
